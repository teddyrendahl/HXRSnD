...
class XPS:
	...
	def __send (self, socketId, command):
		try:
			XPS.__sockets[socketId].send(command)
			return [0,'']
		except socket.timeout:
	...
	def TCP_ConnectToServer (self, IP, port, timeOut):
		socketId = 0
	        #print 'TCP_ConnectToServer: __nbSockets = %s' % (XPS.__nbSockets)
		...
		XPS.__nbSockets += 1
	        #print 'TCP_ConnectToServer: socketId = %s, __nbSockets = %s' % (socketId, XPS.__nbSockets)
		try:
	...
	def GroupMoveAbsoluteNoWait (self, socketId, GroupName, TargetPosition):
		if (XPS.__usedSockets[socketId] == 0):
			return
		command = 'GroupMoveAbsolute(' + GroupName + ','
		for i in range(len(TargetPosition)):
		...
		[error, returnedString] = self.__send(socketId, command)
		return [error, returnedString]
