...
class PV(pycaPv):
  ...
  def __init__(self, name):
    ...
    try:
      ...
      logprint("## WARNING ## connecting to %s timeout, will try once more" % name)
      time.sleep(0.1)
      self.connect(time_out_connect)
...
#        if self.status == pyca.NO_ALARM:
  ...
  def get_with_pycaPVget(self,handle_no_ioc=True):
  ...
  def get(self):
    if (self.ismonitored==False):
      v = self.get_with_pycaPVget()
      self.monitor_start()
      return v
    return self.value
...
def monitor_until_value(pvname,value,timeout=60):
  ...
  t0 =time.time()
  add_pv_to_cache(pvname)
  pv = g_pv_chache_list[pvname]
  pvvalue=pv.get()
  if (pvvalue==value): return
  try:
    while ( (time.time()-t0) < timeout):
      time.sleep(0.010)
      if (g_pv_chache_list[pvname].value==value): break
    if ((time.time()-t0) > timeout):
      print "waiting for pv %s to become %s timedout" % (pvname,value)
  finally:
    if (pv.monitor_append):
      monitor_stop(pvname)
