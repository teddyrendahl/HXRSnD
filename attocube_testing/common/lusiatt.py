...
class Filter(object):
  ...
  def __init__ (self,motor,material,thickness,inpos=26.0,outpos=1.0):
    self.motor  = motor
    self.d   = thickness
...
class Lusiatt(object):
  ...
  """
  def __init__(self,filter_motors,lclsinstance,PVbase=None):
    ...
    #self.att[3] = Filter(filter_motors[3],silicon,20e-6*pow(2,3),inpos=1)
    #self.att[1] = Filter(filter_motors[1],silicon,20e-6*pow(2,1),inpos=22)
    self.att[6] = Filter(filter_motors[6],silicon,20e-6*pow(2,6),inpos=25)
    self.setE()
